...
Except JS-wrapper components can't participate in the server render. They emit an empty `<div>` placeholder during SSR, then the JavaScript loads, initializes the chart engine, and replaces the placeholder. Your users see a blank space — sometimes for a full second or two — then the chart pops in.
...
The output is an SVG `<path>` element. Real DOM. Participates in CSS transitions, responds to Blazor event handlers, works with `@ref`. The browser's GPU handles the painting — which it's been doing well for, what, 15 years now?
