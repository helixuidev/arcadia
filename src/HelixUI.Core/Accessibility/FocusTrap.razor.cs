...
public partial class FocusTrap : Base.HelixComponentBase, IAsyncDisposable
{
    ...
    private string? CssClass => Utilities.CssBuilder.Default("helix-focus-trap")
        .AddClass("helix-focus-trap--active", Active)
        .AddClass(Class)
        .Build();
    ...
    private async Task FocusFirstElementAsync()
    {
        try
        {
            _module ??= await JSRuntime.InvokeAsync<IJSObjectReference>(
                "import", "./_content/HelixUI.Core/js/focusTrap.js");
