...
title: "Your Blazor Components Aren't Actually Blazor. And It's Costing You."
description: "Why the next generation of Blazor UI libraries renders in C#, not JavaScript. SSR flash, bundle size, memory leaks, and the case for native rendering."
date: "2026-03-29"
author: "Arcadia Controls"
tags: ["blazor", "native rendering", "javascript", "SVG", "performance", "SSR"]
---
...
| Native C# approach | less than 12 KB |
...
```
C# Parameter > JSON Serialize > SignalR/WASM bridge > JS Deserialize > Canvas 2D API > Pixels
```

The native version:

```
C# Parameter > BuildRenderTree > SVG path element > Browser GPU > Pixels
```
...
The output is an SVG path element. Real DOM. Participates in CSS transitions, responds to Blazor event handlers, works with @ref. The browser's GPU handles the painting, and it's been doing that well for, what, 15 years now?
...
The full [Arcadia Controls](https://arcadiaui.com) suite has 20 chart types, a DataGrid, forms, and UI components if you want to go deeper. But honestly, just trying the gauge and inspecting what it renders in the DOM is enough to understand the architectural difference. It either clicks for you or it doesn't.
